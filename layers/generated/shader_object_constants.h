...
enum AdditionalExtensionFlagBits {
    ...
    COLOR_WRITE_ENABLE              = 1u << 7,
    GRAPHICS_PIPELINE_LIBRARY       = 1u << 8,
...
inline AdditionalExtensionFlags AdditionalExtensionStringToFlag(const char* pExtensionName) {
    ...
    if (strncmp(pExtensionName, VK_EXT_COLOR_WRITE_ENABLE_EXTENSION_NAME,          VK_MAX_EXTENSION_NAME_SIZE) == 0) { return COLOR_WRITE_ENABLE; }
...
constexpr ExtensionData kAdditionalExtensions[] = {
    ...
    { VK_EXT_COLOR_WRITE_ENABLE_EXTENSION_NAME,          COLOR_WRITE_ENABLE },
    { VK_EXT_GRAPHICS_PIPELINE_LIBRARY_EXTENSION_NAME,   GRAPHICS_PIPELINE_LIBRARY },
};

constexpr uint32_t kMaxDynamicStates = 59;
