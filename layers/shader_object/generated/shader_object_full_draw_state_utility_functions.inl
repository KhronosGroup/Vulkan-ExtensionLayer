...
static void SetInternalArrayPointers(FullDrawStateData* state, Limits const& limits) {
    ...
    state->color_blend_attachment_states_ =
        aligned_memory.GetNextAlignedPtr<VkPipelineColorBlendAttachmentState>(limits.max_color_attachments);
    state->viewport_swizzles_ = aligned_memory.GetNextAlignedPtr<VkViewportSwizzleNV>(limits.max_viewports);
    state->vertex_input_attribute_descriptions_ =
        aligned_memory.GetNextAlignedPtr<VkVertexInputAttributeDescription>(limits.max_vertex_input_attributes);
    state->vertex_input_binding_descriptions_ =
        aligned_memory.GetNextAlignedPtr<VkVertexInputBindingDescription>(limits.max_vertex_input_bindings);
}
