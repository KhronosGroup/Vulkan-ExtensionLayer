...
Extension layers and their tests.
...
The Extension layer tests depend on the Vulkan loader when they execute and
...
`Vulkan-ExtensionLayer.sln` in the build directory.
...
Launch Visual Studio and open the "Vulkan-ExtensionLayer.sln" solution file
...
sanity tests, especially the included Extension Layer tests (vk_extension_layer_tests).
...
The Extension Layers by default build and release for Android 26 (Android Oreo). While Vulkan is supported in Android 24 and 25, there is no AHardwareBuffer support. To build a version of the Extension Layers for use with Android that will not require AHB support, simply addjust the `APP_PLATFORM` in [build-android/jni/Application.mk](build-android/jni/Application.mk)
...
everything in the repository for Android, including Extension layers, tests,
...
Resulting Extension layer binaries will be in build-android/libs. Test and
...
After making any changes to the repository you should perform some quick
sanity tests, including the Extension layer tests.

#### Run Extension Layer Tests

Use the following steps to build, install, and run the extension layer tests
for Android:
...
Alternatively, you can use the test_APK script to install and run the extension layer
tests:
...
vk_extension_layer_tests located in the tests subdirectory.
